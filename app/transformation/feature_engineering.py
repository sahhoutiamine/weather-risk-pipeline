...
class FeatureEngineering:
    ...
    def create_temperature_category(self):
        conditions = [
            (self.df['temp_max'] < 10),
            (self.df['temp_max'] >= 10) & (self.df['temp_max'] < 25),
            (self.df['temp_max'] >= 25) & (self.df['temp_max'] < 35),
            (self.df['temp_max'] >= 35) & (self.df['temp_max'] < 40),
            (self.df['temp_max'] >= 40)
        ]
    ...
    def create_precipitation_category(self):
        conditions = [
            ...
            (self.df['precipitation'] > 0) & (self.df['precipitation'] <= 5),
            (self.df['precipitation'] > 5) & (self.df['precipitation'] <= 20),
            (self.df['precipitation'] > 20) & (self.df['precipitation'] <= 50),
            (self.df['precipitation'] > 50)
        ]
        choices = ["No Rain", "Light Rain", "Moderate Rain", "Heavy Rain", "Extreme Rain"]
        self.df["rain_category"] = np.select(conditions, choices, default="Unknown")

    def create_wind_category(self):
        conditions = [
            self.df["wind_speed"] < 20,
            (self.df["wind_speed"] >= 20) & (self.df["wind_speed"] < 40),
            (self.df["wind_speed"] >= 40) & (self.df["wind_speed"] < 60),
            self.df["wind_speed"] >= 60
        ]
        choices = ["Calm", "Moderate", "Strong", "Extreme"]
        self.df["wind_category"] = np.select(conditions, choices, default="Unknown")

    def create_temperature_range(self):
        self.df["temp_range"] = (self.df["temp_max"] - self.df["temp_min"]).round(1)
    ...
    def create_risk_level(self):
        conditions = [
            self.df["risk_score"] <= 20,
            (self.df["risk_score"] > 20) & (self.df["risk_score"] <= 40),
            (self.df["risk_score"] > 40) & (self.df["risk_score"] <= 70),
            self.df["risk_score"] > 70
        ]
    ...
    def create_operational_flags(self):
        self.df["is_risky"] = self.df["risk_score"] > 40
        self.df["is_heatwave"] = self.df["temp_max"] >= 40
        self.df["is_heavy_rain"] = self.df["precipitation"] > 20
        self.df["is_strong_wind"] = self.df["wind_speed"] >= 40
